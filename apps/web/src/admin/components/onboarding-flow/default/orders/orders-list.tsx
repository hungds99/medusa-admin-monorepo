...
import { useAdminCreateDraftOrder, useAdminProduct, useMedusa } from 'medusa-react';
...
const OrdersListDefault = ({ onNext, isComplete, data }: StepContentProps) => {
  ...
  return (
    <>
      <div className='mb-6'>
        <Text className='mb-2'>
          The last step is to create a sample order using the product you just created. You can then
          view your order’s details, process its payment, fulfillment, inventory, and more.
        </Text>
        <Text>
          By clicking the “Create a Sample Order” button, we’ll generate an order using the product
          you created and default configurations.
        </Text>
      </div>
      <div className='flex gap-2'>
        {!isComplete && (
          <Button variant='primary' size='base' onClick={() => createOrder()} isLoading={isLoading}>
