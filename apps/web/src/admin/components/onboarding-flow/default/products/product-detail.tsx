...
import { useAdminPublishableApiKeys, useAdminCreatePublishableApiKey } from 'medusa-react';
...
const ProductDetailDefault = ({ onNext, isComplete, data }: StepContentProps) => {
  ...
  const backendUrl =
    process.env.MEDUSA_BACKEND_URL === '/' || process.env.MEDUSA_ADMIN_BACKEND_URL === '/'
      ? location.origin
  ...
  return (
    <div>
      <div className='flex flex-col gap-2'>
        ...
        <Text>
          You can preview your product using Medusa's Store APIs. You can copy any of the following
          code snippets to try it out.
        </Text>
      </div>
      <div>
        {!isLoading && (
            ...
            snippets={[
              {
                label: 'cURL',
                language: 'bash',
                code: `curl "${backendUrl}/store/products/${data?.product_id}"${
                  api_key ? ` -H "x-publishable-key: ${api_key}"` : ``
                }`,
              },
              {
                ...
                code: `// Install the JS Client in your storefront project: @medusajs/medusa-js\n\nimport Medusa from "@medusajs/medusa-js"\n\nconst medusa = new Medusa(${
                  api_key ? `{ publishableApiKey: "${api_key}"}` : ``
      ...
      <div className='flex gap-2'>
        <a href={`${backendUrl}/store/products/${data?.product_id}`} target='_blank'>
